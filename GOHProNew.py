...
#------------------------Constructing Modular similarity network---------------------------------------------
def Weight_Complex():
   ...
   return WeightC
#------------------------Constructing Domain structual similarity network------------------------------------
def Weight_Domain():
...
#-----------------------Loading PPI netwok and GO annotation----------------------------------------------- 
def LoadPPI():
...
#----------------------------Loading Protein domain profile and Protein complex-------------------------------      
def LoadMultiData():
   #Loading Domain
...
def GOHPro():
   ...
   def process_one_protein(i):
       ...
       return Scores
   results = Parallel(n_jobs=2)(delayed(process_one_protein)(i) for i in range(LP))
...
def Evaluations(all_predictions,true_go_terms):
    ...
    print(best_fmax,best_smin,micro_auc,micro_aupr)   
    return {
        "fmax": best_fmax,
        "smin": best_smin,
        "auroc": micro_auc,
...
def main():  
    ...
    Evaluations(all_predictions,true_go_terms) 
